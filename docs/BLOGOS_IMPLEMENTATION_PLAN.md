...
| 2 | 完了（実際の si-note での通し確認は、ローカルのSSL証明書の問題を解消した後に行う） | 2026-09-27 |
| 3 | ローカルで完了（XServerでのMigration実行とcronの動作確認は利用者が行う） | 2026-09-26 |
| 4 | 完了（si-note で、下書きのテスト投稿の作成・更新・ゴミ箱・完全削除を確認。WordPress側の拡張は、テーマの作り込みの後に有効になる） | 2026-09-26 |
| 5 | 完了（si-note の実データで GA4・Search Console・AdSense の取得と記事の対応付けを確認。AdSense の記事ごとの収益はAPIから得られない。D-21-09） | 2026-09-26 |
| 6 | 実装済み（品質評価と手動実行。テストと実データの指示文で確認。実際にChatGPTで実行しての確認は利用者が行う。API実行は OpenAI API の契約後） | 2026-09-26 |
