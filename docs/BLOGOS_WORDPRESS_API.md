...
編集案を、実際のテーマ・ショートコード・プラグインで表示して、変更前と比べるための拡張。記事は保存しない。

| 項目 | 内容 |
| --- | --- |
| 作成 | `POST /wp-json/blogos/v1/preview`（`edit_posts` の権限が必要）。`post_id`（WordPressの記事のID。新規記事は 0）、`post_type`（`post` / `page`）、`title`・`content`・`excerpt`（送らない項目は差し替えない。変更前の表示では送らない） |
| 応答 | `url`（表示用のURL）、`shell_post_id`（表示の土台にした記事）、`is_new`、`expires_in` |
| 表示用のURL | `/?p=<ID>&blogos_preview=<トークン>`（固定ページは `page_id`）。トークンは推測できない40文字で、1回だけ使え、10分で期限が切れる（`transient`） |
| 表示の間だけの差し替え | メインのクエリの対象の記事のタイトル・本文・抜粋（`posts_results`）。公開されていない記事も表示する（`pre_get_posts`）。正規のURLへの転送をしない |
| 新規記事 | 同じ種類の最新の公開済みの記事を土台にし、タイトル・本文・抜粋だけを差し替える |
| その他 | 応答に `X-Robots-Tag: noindex, nofollow` と `Cache-Control: no-store`。管理バーを出さない。関数 `blogos_is_preview()` で、プレビューの表示中かを判定できる（子テーマの表示回数の記録で、数えないために使う） |

BlogOSは、表示用のURLのページをサーバーから取得し、スクリプトを取り除いて表示する（アクセス解析・広告に数えない）。スマートフォンの表示は、スマートフォンのブラウザの名乗り（User-Agent）で取得する（テーマがこれで表示を切り替えるため）。
