...
| 日付 | 内容 |
| --- | --- |
| 2026-09-27 | 日本語のスラッグが符号化した形（%e3%…）のまま表示されていたのを直した（決定は D-29）：画面では読める形で表示し、編集案は読める形で扱い、反映・更新の比較では両方の形を同じとみなす。テスト153件が成功 |
| 2026-09-27 | 編集案のプレビューを実装（決定は D-28）：変更前と編集案を、WordPress自身が保存せずに表示したページで左右に並べる（PC・スマホ、スクロールの連動）。WordPress側の拡張にプレビューの仕組み（`POST /wp-json/blogos/v1/preview`、1回だけ使える10分のトークン）を加え、BlogOSはページを取得してスクリプトを取り除いて表示する（GA4・AdSense・表示回数に数えない）。あわせて、si-note のテーマを STINGER8 から独自テーマ（親 Theme-SI-Original・子 Theme-SI-Note）に切り替え、WordPress側の拡張が有効になった。テスト150件が成功 |
